	...
	public class MASImportView : Form
	{
		...
		private void cmdSearchFiles_Click(object sender, EventArgs e)
		{
			try
			{
				...
				var fileInfos = directoryInfo.GetFiles("AR*.xml")
					.Where(f => f.LastWriteTime.Date >= this.dpSelectDate.Value.Date);
				foreach (FileInfo fi in fileInfos)
					_importFiles.Add(new ExtendedFileInfo(fi, "AR"));
				var enumerable = directoryInfo.GetFiles("AP*.xml")
					.Where(f => f.LastWriteTime.Date >= this.dpSelectDate.Value.Date);
				foreach (FileInfo fi2 in enumerable)
					_importFiles.Add(new ExtendedFileInfo(fi2, "AP"));
				_importFiles.ForEach(f => this.lvAvailableFiles.Items.Add(new ListViewItem(new string[2]
				{
					f.FileInformation.Name,
					f.FileInformation.LastWriteTime.ToString()
				})));
				SetListViewRows(lvAvailableFiles);
				cmdSelectAll.Enabled = lvAvailableFiles.Items.Count > 0;
			}
			catch (Exception ex)
			{
		...
		private void ProcessBatch()
		{
			...
			foreach (ListViewItem checkedItem in this.lvAvailableFiles.CheckedItems)
			{
				...
				batchFile.ImportBatchFileRecordCount = !batchFile.ImportBatchFileType.Equals("AR") ? 
                    this.ProcessAPFile(batchFile, extendedFileInfo.FileInformation) : 
                    this.ProcessARFile(batchFile, extendedFileInfo.FileInformation);
		...
		{
			string parameterValue = Path.Combine(Application.StartupPath, "ToMas");
			...
			{
				int tempBatchFileKey = int.Parse(checkedItem.SubItems[2].Text);
				ImportBatchFile importBatchFile = this._context.ImportBatchFiles.Single(bf => bf.ImportBatchFileName == checkedItem.Text);
				if (importBatchFile.ImportAPHeaders.Any())
				{
					if (!flag2)
					{
				...
				if (importBatchFile.ImportARHeaders.Count() > 0)
				{
					if (!flag1)
					{
				...
				importBatchFile.ImportBatchFilePostUser = new Guid?(new Guid("374915F0-0eff-4a9e-bc80-f30c13A07554"));
            ...
            _context.ImportBatchFiles.RemoveRange(_context.ImportBatchFiles.Select(x => x));
